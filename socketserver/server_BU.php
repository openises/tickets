...
if($https) {
	...
	$pemfile = '/etc/letsencrypt/live/www.ticketsaws.net/fullchain.pem';
	$pem_passphrase = "Tx47bPx141";
	if(!file_exists($pemfile)) {
		print "Can't see Certificate File<BR />";
		exit();
		}
...
function handshake($client, $buffer) {
    ...
    $info['ip'] = $address[0];
    $info['port'] = $address[1];
	$secwebsocketkey = str_replace("Sec-WebSocket-Extensions:",'',$header[27]);
	$secwebsocketkey = trim(preg_replace('/\s\s+/', ' ', $secwebsocketkey));;
	$info['Sec-WebSocket-Key'] = $secwebsocketkey;
...
stream_set_timeout($server, 300000);
...
while(true) {
	...
	stream_set_timeout($new_client, 300000);
	if($new_client) {
...
//		$info = handshake($new_client, $buffer, $buffer);
...
//message from existing client
	foreach($read_socks as $key=>$sock) {
		...
		if(!$data){
			...
			unset($users_connected[array_search($sock, $client_socks)]);
			@fclose($sock);
