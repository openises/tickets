...
*/
	...
	$query = "SELECT `chat_messages`.*, UNIX_TIMESTAMP(`when`) AS `when`, `$GLOBALS[mysql_prefix]user`.`user` AS `user_name`, 
		`chat_messages`.`id` AS chat_messages_id 
		FROM `$GLOBALS[mysql_prefix]chat_messages` 
		LEFT JOIN `$GLOBALS[mysql_prefix]user` ON `$GLOBALS[mysql_prefix]chat_messages`.`user_id` = `user`.`id` 
