...
<BODY>
<?php
    $query2 = "SELECT * FROM `{$GLOBALS['mysql_prefix']}capability_types` WHERE `id` = ? LIMIT 1";
    $result2 = db_query($query2, [$row_all['skill_id']]);    $row2 = stripslashes_deep($result2->fetch_assoc());
    ...
    <DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
        <DIV id = "leftcol" style='position: relative; left: 30px; top: 70px; float: left;'>
            <DIV id='button_bar' class='but_container'>
                <SPAN CLASS='heading' STYLE='text-align: center; display: inline; font-size: 1.5em;'>View <?php print get_text('Capability');?> for "<?php print e($row['field2']);?> <?php print e($row['field1']);?>"</SPAN>
