...
function set_size() {
	...
	for (var i = 0; i < fields.length; i++) {
		if($(fields[i])) {$(fields[i]).style.width = smallfieldwidth + "px";}
		} 
	for (var i = 0; i < medfields.length; i++) {
		if($(medfields[i])) {$(medfields[i]).style.width = smallfieldwidth + "px";}
		}
...
var track_captions = ["", "Callsign", "Device key", "Userid ", "Userid ", "Badge", "Device", "Userid", "Automatic", "Callsign", "Device Key", "Callsign"];	//	9/6/13
...
function track_reset(the_Form) {		// reset to original as-loaded values
	...
	the_Form.frm_traccar.value = <?php echo $row['traccar'];?>;				//	9/6/13
	the_Form.frm_javaprssrvr.value = <?php echo $row['javaprssrvr'];?>;				//	9/6/13
	}		// end function track reset()
	
var track_captions = ["", "Callsign&nbsp;&raquo;", "Device key&nbsp;&raquo;", "Userid&nbsp;&raquo;", "Userid&nbsp;&raquo;", "Badge&nbsp;&raquo;", "Device&nbsp;&raquo;", "Userid&nbsp;&raquo;","Automatic&nbsp;&raquo;","Callsign&nbsp;&raquo;","Device Key&nbsp;&raquo;","Callsign&nbsp;&raquo;"];	//	 9/6/13
</SCRIPT>
</HEAD>
...
?>
						<TR CLASS='odd' VALIGN='top'>
							<TD CLASS='td_label text'><A CLASS="td_label text" HREF="#" TITLE="Shows Regions that Responder is allocated to"><?php print get_text("Regions");?></A>:
							<SPAN id='expand_gps' onClick="$('groups_sh').style.display = 'inline-block'; $('expand_gps').style.display = 'none'; $('collapse_gps').style.display = 'inline-block';" style = 'display: inline-block; font-size: 16px; border: 1px solid;'><B>+</B></SPAN>
							<SPAN id='collapse_gps' onClick="$('groups_sh').style.display = 'none'; $('collapse_gps').style.display = 'none'; $('expand_gps').style.display = 'inline-block';" style = 'display: none; font-size: 16px; border: 1px solid;'><B>-</B></SPAN></TD>
							<TD>
<?php
...
?>
				...
				<TR CLASS = "even" VALIGN='middle'>
					...
					<TD>&nbsp;</TD>
					<TD CLASS='td_data text' COLSPAN=2 ALIGN='left'><FONT SIZE='-2'>
						<SELECT ID='unittype' NAME='frm_type'>
<?php
...
?>
						...
						<A CLASS="td_label text" HREF="#" TITLE="Check if Unit is mobile">Mobile</A> &raquo;<INPUT TYPE="checkbox" NAME="frm_mob_disp" <?php print $mob_checked; ?> />&nbsp;&nbsp;&nbsp;
						<A CLASS="td_label text" HREF="#" TITLE="Check if Unit can be dispatched to multiple incidents - e.g., ACO">Multiple</A>  &raquo;<INPUT TYPE="checkbox" NAME="frm_multi_disp" <?php print $multi_checked; ?> />&nbsp;&nbsp;&nbsp;
...
?>
					...
					<TR CLASS='odd' VALIGN="top">	<!--  6/10/11 -->
						<TD CLASS="td_label text">
							<A CLASS="td_label text" HREF="#" TITLE="Sets exclusion zone for this unit"><?php print get_text("Exclusion Zone");?></A>:
						</TD>
						<TD>&nbsp;</TD>
						<TD CLASS='td_data text' COLSPAN=2>
...
<?php
				...
				$usng_link = (!$is_mobile)? "<SPAN CLASS='td_label text' ID = 'usng_link' onClick = 'do_usng_conv(res_edit_Form)'>USNG:</SPAN>": "USNG:";
				$osgb_link = (!$is_mobile)? "<SPAN CLASS='td_label text' ID = 'osgb_link'>OSGB:</SPAN>": "OSGB:";		
				$utm_link = (!$is_mobile)? "<SPAN CLASS='td_label text' ID = 'utm_link'>UTM:</SPAN>": "UTM:";
				if($good_internet) {
?>
					<TR CLASS = "even">
						<TD CLASS="td_label text">
						...
						<TD>&nbsp;</TD>
						<TD CLASS='td_data text' COLSPAN=2>
							<INPUT TYPE="text" ID='show_lat' NAME="show_lat" VALUE="<?php print get_lat($row['lat']);?>" SIZE=11 disabled />&nbsp;
							<INPUT TYPE="text" ID='show_lng' NAME="show_lng" VALUE="<?php print get_lng($row['lng']);?>" SIZE=11 disabled />&nbsp;
<?php
							...
							switch($locale) { 
								case "0":
									print $usng_link;
?>
									<INPUT ID='grid' TYPE="text" NAME="frm_ngs" VALUE="<?php print LLtoUSNG($row['lat'], $row['lng']) ;?>" SIZE=19 disabled />
<?php 	
									break;
								...
								case "1":
									print $osgb_link;
?> 								
									<INPUT ID='grid' TYPE="text" NAME="frm_ngs" VALUE="<?php print LLtoOSGB($row['lat'], $row['lng']) ;?>" SIZE=19 disabled />
<?php 
									break;
...
?> 								
									<INPUT ID='grid' TYPE="text" NAME="frm_ngs" VALUE="<?php print LLtoUTM($utmcoords);?>" SIZE=19 disabled />
<?php 
									break;		
...
<?php
				...
				$todisp = ((is_guest()) || (!(can_do_dispatch($row))))?	"" : "<A id='disp_" . $row['id'] . "' CLASS='plain_centerbuttons' style='float: none; width: 80px; display: block;' HREF='{$_SESSION['unitsfile']}?func=responder&view=true&disp=true&id=" . $row['id'] . "' onMouseOver=\"do_hover_centerbuttons(this.id);\" onMouseOut=\"do_plain_centerbuttons(this.id);\">" . get_text('To Dispatch') . "<BR /><IMG id='disp_img' SRC='./images/dispatch.png' /></A>";
?>
