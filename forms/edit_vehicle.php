...
function set_initial_view(val) {
	...
	var days = "<?php print e($row_all['days']);?>";
...
<BODY onload="pop_veh(<?php print e($row_all['skill_id']);?>); <?php print $startup;?>;">
	<DIV id = "outer" style='position: absolute; left: 0px; width: 90%;'>
		<DIV CLASS='header text_large' style = "height:32px; width: 100%; float: none; text-align: center;">
			<SPAN ID='theHeading' CLASS='header text_bold text_big' STYLE='background-color: inherit;'><b>Edit <?php print get_text('Vehicle');?> Record for "<?php print e($row['field2']);?> <?php print e($row['field1']);?>"</b></SPAN>
		</DIV>
...
<?php
						...
						$result_veh = db_query($query_veh);						while ($row_veh = stripslashes_deep($result_veh->fetch_assoc())) {
...
?>								
				...
				<INPUT TYPE='hidden' NAME = 'frm_start' VALUE="<?php print e($row_all['start']);?>"/>
				<INPUT TYPE='hidden' NAME = 'frm_end' VALUE="<?php print e($row_all['end']);?>"/>
				<INPUT TYPE='hidden' NAME = 'frm_days' VALUE="<?php print e($row_all['days']);?>"/>						
