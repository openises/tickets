...
function createcrossMarker(lat, lon) {
	if((isFloat(lat)) && (isFloat(lon))) {
		var iconurl = "./our_icons/crosshair.png";		
...
function init_map(theType, lat, lng, icon, theZoom, locale, useOSMAP, control_position) {
	if(locale == 1 && useOSMAP == 1) {	//	UK Use Ordnance Survey as Basemap
		...
		map.on('baselayerchange', function (eventLayer) {
			...
			var params = "f_n=layer_inuse&v_n=" + URLEncode(layerName) + "&sess_id=" + window.sess_id;
			var url = "persist3.php";	//	3/15/11	
			sendRequest (url, layer_handleResult, params);	
			});
		} else {
		var latLng;
		var osmUrl = (window.in_local_bool=="1")? "./tiles/{z}/{x}/{y}.png":	"http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png";
		var	cmAttr = '';
		...
		var grid = L.graticule({ interval: .5 })
		var currentSessionLayer = "Open_Streetmaps";
		...
		theLayer = (window.in_local_bool != "1") ? baseLayerVarArr[a]: OSM;	// Load OSM if using local maps
		if(window.geo_provider == 1) {
		...
		if(window.in_local_bool == "0") {
			...
			map.setView([lat, lng], window.setZoom);
			bounds = map.getBounds();	
			zoom = map.getZoom();
...
function init_minimap(theType, lat, lng, icon, theZoom, locale, useOSMAP) {
	...
	var my_Path = "http://127.0.0.1/tiles/";
	var osmUrl = (window.in_local_bool=="1")? "../tiles/{z}/{x}/{y}.png":	"http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png";
	var	cmAttr = 'Map data &copy; 2011 OpenStreetMap contributors, Imagery &copy; 2011 CloudMade';
	var OSM   = L.tileLayer(osmUrl, {attribution: cmAttr});
