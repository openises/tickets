(function (factory) {
	...
	L.Control.Geocoder.getJSON = function(url, params, callback) {
		...
		xmlHttp.onreadystatechange = function () {
			...
			callback(JSON.decode(xmlHttp.response));
		};
	};
	...
	L.Control.Geocoder.Bing = L.Class.extend({
		...
		reverse: function(location, scale, cb, context) {
			L.Control.Geocoder.jsonp('//dev.virtualearth.net/REST/v1/Locations/' + location.lat + ',' + location.lng, {
				...
				for (var i = data.resourceSets[0].resources.length - 1; i >= 0; i--) {
					var address_parts = [];
					for(var z in data.resourceSets[0].resources) {
						for(var zz in data.resourceSets[0].resources[z]) {
							if(zz == "address") {
								for(var zy in data.resourceSets[0].resources[z][zz]) {
									address_parts.push(data.resourceSets[0].resources[z][zz][zy]);
									}
								}
							}
						}
