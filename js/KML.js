...
L.Util.extend(L.KML, {
	...
	parseStyle: function (xml) {
		...
		for (var i = 0; i < sl.length; i++) {
			...
			if (poptions.opacity) { options.fillOpacity = poptions.opacity; } else { options.fillOpacity = "0.01";}
			if (poptions.fill) {
				if(poptions.fill = 0) {
					options.fill = false;
					} else {
	...
	parsePlacemark: function (place, xml, style) {
		var s, h, i, j, el, options = {};
		var extended = ['ExtendedData'];
		for (s in extended) {
			el = place.getElementsByTagName(extended[s]);
		...
		var name, k, descr = '', extra = '';
		el = place.getElementsByTagName('name');
		if (el.length && el[0].childNodes.length) {
			name = "Name: " + el[0].childNodes[0].nodeValue;
			alert(name)
		}
		...
		for (i = 0; i < el.length; i++) {
			for (j = 0; j < el[i].childNodes.length; j++) {
				for (k = 0; k < el[i][j].childNodes.length; k++) {
					extra = extra + el[i][j].childNodes[k].nodeValue;
					alert(extra);
				}
			}
		}
