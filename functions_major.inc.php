...
//					gmarkers[i].hide();						// hide incidents - 1/8/09
	...
	var ignore = false;						// global re bubble cancel - 12/17/09
	...
	function myclick(id) {					// Responds to sidebar click, then triggers listener above -  note [i]
		if (ignore) {			// bubble cancel - 12/17/09
			ignore = false;
			return;
			}
...
//		starting = false;
	...
	side_bar_html += "<TR class='even'><TH colspan=99 align='center'>Click for information</TH></TR>";
	side_bar_html += "<TR class='odd'><TH></TH><TH align='left' COLSPAN=2>Incident</TH><TH align='left'>Type</TH><TH align='left'>P</TH><TH align='left'>A</TH><TH align='left'>As of</TH></TR>";
	var gmarkers = [];
	var fmarkers = [];
	var infoTabs = [];
	var facinfoTabs = [];
...
<?php
	...
	//		$tab_2 .= "<TR>					<TD>&nbsp;</TD></TR>";
			...
			$sidebar_line = "<TD CLASS='$severityclass'  TITLE = '" . htmlentities ($row['scope'], ENT_QUOTES) . "' COLSPAN=2><NOBR>$strike" . $sp . shorten($row['scope'], 20) . " $strikend</NOBR></TD>";	//10/27/09
...
//	$query = "SELECT *, UNIX_TIMESTAMP(updated) AS `updated` FROM `$GLOBALS[mysql_prefix]responder` ORDER BY `handle`";	//
	...
	$result = mysql_query($query) or do_error($query, 'mysql query failed', mysql_error(), basename( __FILE__), __LINE__);
	print (mysql_affected_rows()==0)? "\n\t\tside_bar_html += \"<TR CLASS='odd'><TH></TH><TH ALIGN='center' COLSPAN=99>No units!</TH></TR>\"\n" : "\n\t\tside_bar_html += \"<TR CLASS='even'><TH></TH><TH ALIGN='left'>Unit</TH><TH ALIGN='left'>Status</TH><TH ALIGN='left' COLSPAN=2>Dispatch</TH><TH ALIGN='left'>M</TH><TH></TH></TR>\"\n" ;
...
// name
		...
		$temp = explode("/", $name );
		$display_name = $temp[0];
//									dump($row);  ["un_status_id"]
//		$sidebar_line = "<TD TITLE = '" . htmlentities ($row['name'], ENT_QUOTES) . "'><U>" . shorten($row['name'], 24) ."</U></TD>";
		$stat_sel = get_un_stat_sel($row['id'], $row['un_status_id']) ;

		$sidebar_line = "<TD TITLE = '" . htmlentities ($display_name, ENT_QUOTES) . "'><U>" . shorten($display_name, 10) . "</U></TD>";	//	10/8/09
		$sidebar_line .= "<TD>" . get_un_stat_sel($row['id'], $row['un_status_id']) . "</TD>";		// status
...
//		dump($query);
		...
		$ass_td =  (mysql_affected_rows()>0)? 
			"<TD COLSPAN=2 CLASS='$severityclass' TITLE = '" .$row_assign['scope'] . "' >" .$tick_ct . shorten($row_assign['scope'], 24) . "</TD>":
			"<TD>na</TD>";
...
<?php
	...
	if (my_is_float($row['lat'])) {		// map position?
...
<?php
	...
	print "\n\tside_bar_html+= \"<TR CLASS='\" + colors[i%2] +\"'><TD COLSPAN=6 ALIGN='center'>{$source_legend}</TD></TR>\";\n";
