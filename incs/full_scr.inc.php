...
//	snap(basename(__FILE__), __LINE__);
	...
	if ((empty($closed)) && (empty($special))) {		// 9/29/09
...
//	$heading = $closed ? "Closed Incidents" : "Current Situation";		// 3/2/09
	...
	$query = "SELECT * FROM `$GLOBALS[mysql_prefix]ticket` WHERE `status` = 2 ";		// 10/26/09
	...
	$query = "SELECT * FROM `$GLOBALS[mysql_prefix]ticket` WHERE `status` = 3 ";		// 10/26/09
...
<TABLE BORDER=0>
	...
	<TR CLASS='odd'><TD COLSPAN='99' ALIGN='CENTER'><TABLE>	<!-- 10/26/09 -->
<?php
	if(empty($open)) {									// 10/26/09  added button
		echo "<TD ALIGN='center'><INPUT TYPE='button' VALUE='Current situation' onClick = 'document.to_all.submit()'>";
		}
	if((empty($closed)) && ($num_closed > 0)) {					// 10/26/09  added button, 10/21/09 added check for closed incidents on the database
		echo "<TD ALIGN='center'><INPUT TYPE='button' VALUE='Closed Incidents' onClick = 'document.to_closed.submit()'>";
		}
	if((empty($scheduled)) && ($num_scheduled > 0)) {					// 10/26/09  added button for special incidents, 10/21/09 added check for special incidents on the database
		echo "<TD ALIGN='center'><INPUT TYPE='button' VALUE='Scheduled Incidents' onClick = 'document.to_scheduled.submit()'>";
		}
?>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE='button' onClick = "window.close();" VALUE='Finished'> <!-- 10/26/09 -->
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE='button' onClick = "javascript:opener.focus()" VALUE='Back'>

	</TD></TR>

	</TABLE></TD></TR>
	<TR CLASS='odd'><TD COLSPAN='99' ALIGN='center'>&nbsp;</TD></TR>
	<TR><TD VALIGN='TOP' width='1px' ><DIV ID='side_bar'></DIV></TD>
		<TD></TD>
		<TD CLASS='td_label'>
		...
		Units:<IMG SRC = './icons/sm_white.png' BORDER=0><IMG SRC = './icons/sm_black.png' BORDER=0>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	<!-- 10/26/09 -->
		...
		Facilities:<IMG SRC = './icons/sm_shield_green.png' BORDER=0><IMG SRC = './icons/sm_square_red.png' BORDER=0>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	<!-- 10/26/09 -->
		...
		Incident Priority:<IMG SRC = './icons/sm_blue.png' BORDER=0><IMG SRC = './icons/sm_green.png' BORDER=0><IMG SRC = './icons/sm_red.png' BORDER=0>&nbsp;&nbsp;	<!-- 10/26/09 -->
...
//	$where = ($get_status==2)? " WHERE `status`='2' OR `status`='3' OR (`status`='1'  AND `problemend` >= '{$time_back}') ": " WHERE `status`='1' ";	// 4/2/09 9/25/09
	...
	switch($get_status) {				//9/29/09 Added capability for Special Incidents 10/27/09 changed to bring scheduled incidents to front when due.
