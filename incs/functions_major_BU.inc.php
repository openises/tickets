...
//		dump(__LINE__);
?>
		do_sidebar_nm ("<?php print $sidebar_line;?>", i, <?php print $row['id'];?>, sym);		// (sidebar, line_no, rcd_id, letter)
<?php
			}
...
//	$toroute = (is_guest())? "": "&nbsp;<A HREF='routes.php?ticket_id=" . $the_id . "'><U>Dispatch</U></A>";	// 8/2/08
	...
	if ((my_is_float($row_fac['lat'])) && (my_is_float($row_fac['lng']))) {
		...
		$sidebar_fac_line = "<TD TITLE = '" . addslashes($facility_display_name) . "'><U>" . addslashes(shorten($facility_display_name, 24)) ."</U></TD>";
		$sidebar_fac_line .= "<TD>&nbsp;&nbsp;&nbsp;" . addslashes($row_fac['fac_type_name']) ."</TD><TD>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>";
...
//dump($row);
	...
	if ($print == 'true') {
		...
		print "<TR><TD CLASS='print_TD'><B>Pre-booked Date:</B></TD>		<TD CLASS='print_TD'>" . format_date($row['booked_date']) . "</TD></TR>\n";	// 10/6/09
...
function do_ticket($theRow, $theWidth, $search=FALSE, $dist=TRUE) {						// returns table
	...
	$print .= "<TR CLASS='odd' ><TD>Protocol:</TD> <TD CLASS='{$severityclass}'>{$theRow['protocol']}</TD></TR>\n";		// 7/16/09
	...
	$print .= "<TR CLASS='odd'><TD>Pre-booked Date:</TD>		<TD>" . format_date($theRow['booked_date']) . "</TD></TR>\n";	// 10/6/09
