...
<?php
	...
	case "c2" :				// insert new data
		...
		$name = sanitize_string ( $_POST['f_2'] ) ;
...
<?php
	...
	case "u2" :				// update
		...
		$name = sanitize_string ( $_POST['f_0'] ) . "/" . strval ( $_POST['ics_id'] ) ;
		...
		$result = db_query ( $query, array ( $name, $payload, $script, $user_id, $_SERVER['REMOTE_ADDR'], $now_ts, $ics_id ) ) or do_error ( $query, 'mysql query failed', db()->error , basename ( __FILE__ ) , __LINE__ ) ;
?>
<body onload = 'setTimeout ( function () { document.dummy.submit () }, 4000 ) ;' >		<!-- <?php echo __LINE__ ;?> -->
...
//			dump ( __LINE__ ) ;
			...
			$query = "INSERT INTO `{$GLOBALS['mysql_prefix']}ics` ( `name`, `type`, `payload`, count, `_by`, `_from`, `_as-of`, `_sent` ) VALUES
															 ( ?, ?, ?, 0, ?, ?, ?, NULL ) ; ";
			$result = db_query ( $query, array ( $name, FORM, $payload, $user_id, $_SERVER['REMOTE_ADDR'], $now_ts ) ) or do_error ( $query, 'mysql query failed', db()->error , basename ( __FILE__ ) , __LINE__ ) ;
			$temp = db()->insert_id ;				// append id # in order to enforce uniqueness
