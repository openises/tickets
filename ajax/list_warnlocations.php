...
$screen = (!(array_key_exists('screen', $_GET))) ? "situation" : sanitize_string($_GET['screen']);
...
$locations = db_affected_rows()>0 ?  db_affected_rows(): "<I>none</I>";
unset($result);
...
$result = db_query($query);
$num_locations = db_affected_rows();
$i=1;				// counter
// =============================================================================
$utc = gmdate ("U");
if($num_locations == 0) {
	...
	while ($row = stripslashes_deep($result->fetch_assoc())) {		// ==========  major while() for Location ==========
		...
		if(is_guest() || $screen == "fullscreen") {
			...
			$toedit = "<SPAN id='editbut' CLASS='plain text' style='float: none;' onMouseover='do_hover(this.id);' onMouseout='do_plain(this.id);' onClick='doEdit(" . intval($row['id']) . ");'>Edit</SPAN>" ;
			$toview = "<SPAN id='viewbut' CLASS='plain text' style='float: none;' onMouseover='do_hover(this.id);' onMouseout='do_plain(this.id);' onClick='doView(" . intval($row['id']) . ");'>View</SPAN>";
			}
	...
	// tab 1
		...
		if (my_is_float($row['lat'])) {
			...
			$tab_1 .= "<TR CLASS='odd'><TD CLASS='td_label text text_left'>As of:&nbsp;</TD><TD CLASS='td_data text text_left'>" . format_date(safe_strtotime($row['_on'])) . "</TD></TR>";
