...
function assignment_list($id) {
	...
	while ($row = stripslashes_deep($result->fetch_assoc())) {
		...
		if (is_date($time_disp)) {
			...
			$disp_button = "<INPUT ID='disp_btn' TYPE= 'button' aria-label='Not Dispatched yet' tabindex=2 CLASS='btn_not_chkd text_big' VALUE='Dispatched' onClick = 'set_assign(\'d\', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");' STYLE = 'display:" . $display_val . ";' />";
			}
		if (is_date($time_resp)) {
			...
			$resp_button ="<INPUT ID='resp_btn' TYPE= 'button' aria-label='Not Responding yet' tabindex=3 CLASS='btn_not_chkd text_big' VALUE='Responding' onClick = \"set_assign('r', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");\" STYLE = 'display:" . $display_val . ";' />";
			}
		if (is_date($time_onsc)) {
			...
			$on_scene_button ="<INPUT ID='onsc_btn' TYPE= 'button' aria-label='Not on-scene yet' tabindex=4 CLASS='btn_not_chkd text_big' VALUE='On-scene' onClick = \"set_assign('s', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");\" STYLE = 'display:" . $display_val . ";' />";
			}
		if (is_date($time_fenr)) {
			...
			$fenr_button ="<INPUT ID='f_enr_btn' TYPE= 'button' aria-label='Not yet en-route to Facility' tabindex=5 CLASS='btn_not_chkd text_big' VALUE=\"Fac'y enroute\" onClick = \"set_assign('e', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");\" STYLE = 'display:" . $display_val . ";' />";
			}
		if (is_date($time_farr)) { 		// 5/19/11
			...
			$farr_button ="<INPUT ID='f_arr_btn' TYPE= 'button' aria-label='Not yet arrived at Facility' tabindex=6 CLASS='btn_not_chkd text_big' VALUE=\"Fac'y arrive\" onClick = \"set_assign('a', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");\" STYLE = 'display:" . $display_val . ";' />";
			}
		if (is_date($time_clear)) {
			...
			$clear_button ="<INPUT ID='clear_btn' TYPE= 'button' aria-label='Not yet cleared from call' tabindex=7 CLASS='btn_not_chkd text_big' VALUE='Clear' onClick = \"set_assign('c', " . intval($assignID) . ", " . intval($ticketID) . ", " . intval($responderID) . ");\" STYLE = 'display:" . $display_val . ";' />";
