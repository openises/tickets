...
$query = "SELECT * FROM `{$GLOBALS['mysql_prefix']}allocates` WHERE `type`= 4 AND `resource_id` = ? ORDER BY `id` ASC";    //    6/10/11
$result = db_query($query, [$user_id]);    //    6/10/11
$al_names = "";
$a_gp_bounds = array();
...
$result = db_query($query, $where2_params);
...
$result = db_query($query, array_merge($where2_params, $where2_params));
$y = $result->num_rows;
...
while ($row = $result->fetch_assoc()) {
    ...
    $query_01 = "SELECT *,
                ...
                WHERE `ticket_id` = ? AND (`dispatched` IS NOT NULL OR DATE_FORMAT(`dispatched`,'%y') != '00')";
    $result_01 = db_query($query_01, [$tick_id]);
...
while ($row = $result->fetch_assoc()) {
    ...
    $query_01 = "SELECT *,
                ...
                FROM `{$GLOBALS['mysql_prefix']}assigns`
                WHERE `ticket_id` = ? AND (`dispatched` IS NOT NULL OR DATE_FORMAT(`dispatched`,'%y') != '00')";
    $result_01 = db_query($query_01, [$f_tick_id]);
    if($result_01->num_rows > 0) {
        while ($row_01 = $result_01->fetch_assoc()) {
...
while ($row = $result->fetch_assoc()) {
    ...
    $query_01 = "SELECT *,
                ...
                FROM `{$GLOBALS['mysql_prefix']}assigns`
                WHERE `ticket_id` = ? AND (`responding` IS NOT NULL OR DATE_FORMAT(`responding`,'%y') != '00')";
    $result_01 = db_query($query_01, [$tick_id]);
    while ($row_01 = $result_01->fetch_assoc()) {
        $disptime = $row_01['dispatched'];
...
while ($row = $result->fetch_assoc()) {
    ...
    $query_01 = "SELECT *,
                ...
                FROM `{$GLOBALS['mysql_prefix']}assigns`
                WHERE `ticket_id` = ? AND (`on_scene` IS NOT NULL OR DATE_FORMAT(`on_scene`,'%y') != '00')";
    $result_01 = db_query($query_01, [$tick_id]);
    while ($row_01 = $result_01->fetch_assoc()) {
        $disptime = $row_01['dispatched'];
...
$result = db_query($query, $where2b_params);
while ($row = $result->fetch_assoc()) {
    ...
    foreach($resp_ids_na as $value) {
        $query= "SELECT * FROM `{$GLOBALS['mysql_prefix']}responder`
                ...
                WHERE `{$GLOBALS['mysql_prefix']}responder`.`id` = ? AND `{$GLOBALS['mysql_prefix']}un_status`.`hide` = 'n'";
