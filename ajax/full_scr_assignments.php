...
//    ============================= Regions stuff
        ...
        $result_un = db_query($query_un, [$row['unit_id']]);    // 5/4/11
        $un_groups = array();
...
//    ============================= end of Regions stuff
        ...
        if ($inviewed > 0) {    //    Tests to see whether assigned unit is in one of the users groups 5/4/11
            $the_descr = (empty($row['tick_descr'])) ? "&nbsp;" : e(str_replace($eols, " ", $row['tick_descr']));
            $the_short_one = (empty($row['tick_descr'])) ? "&nbsp; " : shorten(e(str_replace($eols, " ", $row['tick_descr'])), 25);

            $address = (empty($row['tick_street']))? "&nbsp;" : $row['tick_street'] . ", ";        // 8/10/10
            $address = e($address . $row['tick_city']. "&nbsp;". $row['tick_state']);
            if (!(empty($row['tick_scope']))) {
                $the_name = e($row['tick_scope']);                                                            // 9/12/09
            ...
            $result_type = db_query($query, [$row['unit_type']]) or do_error($query, 'mysql query failed', '', basename(__FILE__), __LINE__);
            $row_type = (db_affected_rows() > 0) ? stripslashes_deep($result_type->fetch_assoc()) : "";
            ...
            $unit_name = empty($row['unit_id']) ? "[#{$row['unit_id']}]" : e($row['unit_name']) ;            // id only if absent
