...
function can_do_dispatch($the_row) {
	...
	$result_temp = db_query($query, [$the_row['unit_id']]) or do_error($query, 'mysql query failed', '', basename( __FILE__), __LINE__);
...
$result_as = db_query($query) or do_error($query, 'mysql query failed', '', basename( __FILE__), __LINE__);
...
if ($track_type > 0 ) {				// get most recent mobile track data
	...
	$result_tr = db_query($query, [$row['callsign']]) or do_error($query, 'mysql query failed', '', basename( __FILE__), __LINE__);
...
if(array_key_exists ($row['unit_id'] , $assigns_ary)) {
	...
	$result_as = db_query($query, [$row['unit_id']]) or do_error($query, 'mysql query failed', '', basename( __FILE__), __LINE__);
...
$statusTemp = ($row['status_about'] != "") ? e($row['status_about']): "";
...
if (my_is_float($lat)) {										// position data? 4/29/09
	...
	$temp_array[0] = $lat;
	$temp_array[1] = $lng;
	$temp_array[2] = e(shorten($name, 48));
	$temp_array[3] = e(shorten(str_replace($eols, " ", $row['unit_descr']), 256));
	...
	$tab_1 .= "<TR CLASS='even'><TD CLASS='td_data text text_center' COLSPAN=2><B>" . safe_htmlentities(shorten($row['name'], 48),ENT_QUOTES) . "</B> - " . e($the_type) . "</TD></TR>";
	$tab_1 .= "<TR CLASS='odd'><TD CLASS='td_label text text_left'>Description:</TD><TD CLASS='td_data text text_left'>" . safe_htmlentities(shorten(str_replace($eols, " ", $row['description']), 32), ENT_QUOTES) . "</TD></TR>";
	$tab_1 .= "<TR CLASS='even'><TD CLASS='td_label text text_left'>Status:</TD><TD CLASS='td_data text text_left'>" . e($the_status) . " </TD></TR>";
	$tab_1 .= "<TR CLASS='odd'><TD CLASS='td_label text text_left'>Contact:</TD><TD CLASS='td_data text text_left'>" . e($row['contact_name']). " Via: " . e($row['contact_via']) . "</TD></TR>";
	$tab_1 .= "<TR CLASS='even'><TD CLASS='td_label text text_left'>As of:</TD><TD CLASS='td_data text text_left'>" . format_date_2(safe_strtotime($the_time)) . "</TD></TR>";		// 4/11/10
	if ($units_assigned > 0) {
		$tab_1 .= "<TR CLASS='even'><TD CLASS='emph'>Dispatched to:</TD><TD CLASS='emph'><A HREF='main.php?id=" . intval($tickets[$row['unit_id']]) . "'>" . e($ass_td) . "</A></TD></TR>";
		}
	$tab_1 .= "</TABLE></TD></TR>";
	$tab_1 .= "</TABLE>";
...
// tab 2
	if ($row_track) {		// do all three tabs
		...
		$tab_2 .="<TR CLASS='even'><TD CLASS='td_data text text_center' COLSPAN=2><B>" . e($row_track['source']) . "</B></TD></TR>";
		$tab_2 .= "<TR CLASS='odd'><TD CLASS='td_label text text_left'>Course: </TD><TD CLASS='td_data text text_left'>" . e($row_track['course']) . ", Speed:  " . e($row_track['speed']) . ", Alt: " . e($row_track['altitude']) . "</TD></TR>";
		$tab_2 .= "<TR CLASS='even'><TD CLASS='td_label text text_left'>Closest city: </TD><TD CLASS='td_data text text_left'>" . e($row_track['closest_city']) . "</TD></TR>";
		$tab_2 .= "<TR CLASS='odd'><TD CLASS='td_label text text_left'>Status: </TD><TD CLASS='td_data text text_left'>" . e($row_track['status']) . "</TD></TR>";
	...
	$tab_3 .= "<TR><TD class='td_label text text_left'>Lat</TD><TD class='td_data text text_left'>" . e($lat) . "</TD></TR>";
	$tab_3 .= "<TR><TD class='td_label text text_left'>Lng</TD><TD class='td_data text text_left'>" . e($lng) . "</TD></TR>";
