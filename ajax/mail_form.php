...
$tik_id = (array_key_exists('ticket_id', $_GET)) ? sanitize_int($_GET['ticket_id']) : 0;
$responder_id = (array_key_exists('responder_id', $_GET)) ? sanitize_int($_GET['responder_id']) : null;
...
switch($func) {
    case 'doselect':        //    Show Incident Selection
        ...
        $result = db_query($query, [$GLOBALS['STATUS_OPEN'], $GLOBALS['STATUS_SCHEDULED']]);
        $no_tickets = db_affected_rows();
        $return .= "<CENTER>";
        ...
        $return .= "<LABEL for='frm_sel_inc' onmouseout='UnTip()' onmouseover=\"Tip('Select units assigned to specific Incident');\"><EM>". get_text("Units"). " assigned to ". get_text("Incident") . "</EM></LABEL>:
            ...
            while($row = stripslashes_deep($result->fetch_assoc())){
                ...
                if(!(empty($row['scope']))) {                // 6/28/09
                    $return .= "\t<OPTION VALUE='" . intval($row['incident']) . "' STYLE='background-color:{$bg_color}; color:black;' >" . e($row['scope']) . " </OPTION>\n";
                    }
                }
        $return .= "</SELECT>";
        $return .= "</DIV>";
    ...
    case 'all_units':
        ...
        $lines = db_affected_rows() +8;
        $no_rows = $result->num_rows;
        $return = "<FORM ID='contact_form' NAME='contact_form' METHOD='post' ACTION='./ajax/form_post.php?q=". e($sess_id) . "&function=contact'>";
        ...
        if($no_rows>0) {
            ...
            foreach($the_arr as $val) {
                ...
                $smsg = $cell = "";
                $e_add = (($val['contact_via'] == null) || ($val['contact_via'] == "")) ? "<SPAN TITLE='No email address stored' class='cell text' style='color: LightGrey;'>(E) NONE</SPAN>" : "<SPAN TITLE='" . e($val['fullcontactvia']) . "' class='cell text'>(E) " . e($val['contact_via']) . "</SPAN>\n" ;
                if($using_smsg && $smsg_provider == "SMS Responder") {
                    $smsg = (($val['smsg_id'] == null) || ($val['smsg_id'] == "")) ? "<SPAN TITLE='SMS Gateway ID stored' class='cell text' style='color: LightGrey; display:'>(SMSG) NONE</SPAN>" : "<SPAN TITLE='" . e($val['fullsmsgids']) . "' class='cell text'>(SMSG) " . e($val['smsg_id']) . "</SPAN>\n" ;
                    }
                if($using_smsg && $smsg_provider == "Txt Local") {
                    $cell = (($val['cellphone'] == null) || ($val['cellphone'] == "")) ? "<SPAN TITLE='No Cellphone number stored' class='cell text' style='color: LightGrey;'>(CELL) NONE</SPAN>" : "<SPAN TITLE='" . e($val['fullcells']) . "' class='cell text'>(CELL) " . e($val['cellphone']) . "</SPAN>\n" ;
                    }
                $dist = (round($val['distance'],1) != 0) ? "Dist: " . round($val['distance'],2) : "";
                $return .= "\t<SPAN class='row' STYLE='background-color:" . e($val['bg_color']) . "; color:" . e($val['text_color']) . ";'>
                    <SPAN class='cell text'><INPUT TYPE='checkbox' NAME='cb{$i}' VALUE='" . e($val['contact_via']) . ":" . intval($val['responder_id']) . ":" . e($val['smsg_id']) . ":' {$checked}></SPAN>
                    <SPAN class='cell text'>" . $dist . "</SPAN>
                    <SPAN class='cell text' TITLE='" . e($val['status']) . "'>" . e($val['handle']) . "</SPAN>
                    <SPAN class='cell text' TITLE='" . e($val['fullnames']) . "'>" . e($val['name']) . "</SPAN>
                    {$e_add}
                    {$smsg}
                    {$cell}
                    </SPAN>\n";
        ...
        $return .= "<TR VALIGN='top' CLASS='even'><TD CLASS='td_label' ALIGN='right'>Subject: </TD><TD><INPUT TYPE = 'text' NAME = 'frm_subj' SIZE = 55 VALUE='" . e($default_msg) . "'></TD></TR>";
        ...
        $result = db_query($query);
        while ($row = $result->fetch_assoc()) {
            $return .= "\t<OPTION VALUE='" . e($row['code']) . "'>" . e($row['code']) . "|" . e($row['text']) . "</OPTION>\n";
            }
        $return .= "</SELECT>";
        $return .= "<BR />";
        ...
        $return .= "<INPUT type='hidden' NAME='frm_ticket_id' VALUE='" . intval($tik_id) . "' />";
    ...
    case 'selected':
        $tik_id = sanitize_int($_GET['ticket_id']);
        ...
        $query_ass = "SELECT *,
            ...
            LEFT JOIN `$GLOBALS[mysql_prefix]un_status`     `s` ON (`r`.`un_status_id` = `s`.`id`)
            LEFT JOIN `$GLOBALS[mysql_prefix]ticket` `t` ON (`a`.`ticket_id` = `t`.`id`)
            WHERE `t`.`id` = ? AND (LOCATE('@', `contact_via`) > 1 || (`smsg_id` IS NOT NULL AND `smsg_id` <> '')) AND ((`clear` IS NULL) OR (DATE_FORMAT(`clear`,'%y') = '00'))
            ORDER BY `r`.`id` ASC ";
