...
$smsgaddrs = sanitize_string(isset($_GET['smsgaddrs']) ? $_GET['smsgaddrs'] : "");
$the_other = sanitize_string(isset($_GET['other']) && $_GET['other'] != "" ? $_GET['other'] : "");
...
$output .= "<TR CLASS='even'>";
$output .= "<TD COLSPAN=2>";
$output .= "<TEXTAREA CLASS='text' NAME='frm_text' COLS=80 ROWS=" . count($temp)*1.1 . " wrap='soft'>" . e($text) . "</TEXTAREA>";
$output .= "</TD>";
$output .= "</TR>";
...
$output .= "</TR>";
$output .= "</TABLE>";
$output .= "<INPUT TYPE='hidden' NAME = 'ticket_id' VALUE='" . e($ticket_id) . "'/>";
$output .= "<INPUT TYPE='hidden' NAME = 'frm_title' VALUE='" . e($row['scope']) . "'/>";
$output .= "</FORM>";
$ret_arr[] = $output;
print json_encode($ret_arr);