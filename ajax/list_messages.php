...
$facility_id = (isset($_GET['facility_id'])) ? sanitize_int($_GET['facility_id']) : null;
...
if(isset($ticket_id)) { $where .= " AND (`ticket_id` = ?)"; $params[] = $ticket_id; }
if(isset($responder_id)) { $where .= " AND (`resp_id` = ?)"; $params[] = $responder_id; }
if(isset($facility_id)) { $where .= " AND (`resp_id` = ?)"; $params[] = $facility_id; }
if((isset($filter)) && ($filter != "")) {
    $where .= " AND ((`m`.`fromname` REGEXP ?) OR (`m`.`message` REGEXP ?) OR (`m`.`recipients` REGEXP ?) OR (`m`.`subject` REGEXP ?))";
    $params[] = $filter;
    $params[] = $filter;
    $params[] = $filter;
    $params[] = $filter;
...
$allowed_sorts = array('date', 'read_status', 'fromname', 'subject', 'message', 'msg_type', 'recipients', 'id');
if(isset($sort) && in_array($sort, $allowed_sorts, true)) {
    $order = "ORDER BY `{$sort}`";
} else {
    $order = "ORDER BY `read_status`, `date`";
}

// Whitelist sort direction
$order2 = (isset($way) && strtoupper($way) === 'ASC') ? 'ASC' : 'DESC';
...
$query = "SELECT *, `date` AS `date`, `_on` AS `_on`,
        ...
        {$where} {$order} {$order2}";
$result = db_query($query, $params) or do_error('', 'mysql query failed', '', basename( __FILE__), __LINE__);
$num=$result->num_rows;
