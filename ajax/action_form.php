...
$get_action = (empty($_GET['action']))? "form" : sanitize_string($_GET['action']);
$tick_id = (isset($_REQUEST['ticket_id'])) ? sanitize_int($_REQUEST['ticket_id']) : "";
...
if ($get_action == 'edit') {
	...
	$do_yr_asof = true;
	$heading = "Edit Action";
	$output .= "<FORM METHOD='post' NAME='ed_frm' ACTION./ajax/form_post.php?id=" . e($get_id) . "&ticket_id=" . e($tick_id) . "&q=". e($sess_id) . "&function=editaction'>";
	$output .= "<TABLE BORDER='0'>";
	$output .= "<TR CLASS='header'>";
	$output .= "<TD COLSPAN='99' ALIGN='center'>";
	$output .= "<FONT CLASS='header' STYLE='background-color: inherit;'>" . e($heading) . "</FONT>";
	$output .= "</TD>";
	$output .= "</TR>";
	$output .= "<TR CLASS='even' VALIGN='top'><TD><B>Description:</B> <font color='red' size='-1'>*</font></TD>";
	$output .= "<TD colspan=3><TEXTAREA ROWS='4' COLS='60' NAME='frm_description' WRAP='virtual'>" . e($row['description']) . "</TEXTAREA>";
	$output .= "</TD></TR>";
	...
	while ($row = stripslashes_deep($result->fetch_assoc())) {
		...
		$output .= "\t<INPUT TYPE = 'checkbox' VALUE='" . e($row['unit_id']) . "' NAME = '" . e($the_name) . "' " . $checked . " />";
		$output .= "<SPAN STYLE='width:300px; display:inline; background-color:" . e($the_bg_color) . "; color:" . e($the_text_color) . ";'>" . e(stripslashes($row['unit_name'])) . "&nbsp;</SPAN>" . $ct_str;
		$output .= "&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;<SPAN STYLE = 'width:200px; background-color:" . e($row['bg_color']) . "; color:" . e($row['text_color']) . ";'>" . e($row['stat_descr']) . "</SPAN><BR />\n";
		}
	unset ($row);
	$output .= "</DIV>";
	$output .= "</TD>";
	...
	$output .= "<FORM ID='act_form' NAME='act_form' METHOD='post' ACTION='./ajax/form_post.php?q=". e($sess_id) . "&function=addaction'>";
	...
	while ($row = stripslashes_deep($result->fetch_assoc())) {
		...
		$ct_str = ($row['nr_assigned']==0) ? ""  : "&nbsp;({$row['nr_assigned']})" ;
		$the_name = "frm_cb_" . stripslashes($row['unit_name']);
		$output .= "<SPAN class='row' STYLE='background-color: " . e($type_bg_color) . "; color: " . e($type_text_color) . "; width: 98%;'>";
		$output .= "<SPAN class='cell text'><INPUT TYPE = 'checkbox' VALUE='" . e($row['unit_id']) . "' NAME = '" . e($the_name) . "' /></SPAN>";
		$output .= "<SPAN class='cell text'>" . e(stripslashes($row['unit_name'])) . "</SPAN>";
		$output .= "<SPAN class='cell text'>" . $ct_str . "</SPAN>";
		$output .= "<SPAN class='cell text'> - " . e($row['stat_descr']) . " - </SPAN>";
		$output .= "</SPAN>\n";
		}
	$output .= "</DIV><BR /><BR />";
	$output .= "<LABEL FOR='frm_year_asof'>As Of: &nbsp;&nbsp;</LABEL><BR />";
	$output .= return_date_dropdown('asof',0,FALSE);
	$output .= "<INPUT TYPE='hidden' NAME = 'frm_ticket_id' VALUE = '" . e($tick_id) . "' />";
...
$output .= "<INPUT TYPE='hidden' NAME = 'id' VALUE = '" . e($tick_id) . "'>";
$output .= "</FORM></DIV>";
