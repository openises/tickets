...
if(is_guest() || is_unit()) {
    ...
    if ((may_email()) && (is_email($row_fac['contact_email']))) {
        ...
        $tomail = "<SPAN id='mail_" . intval($row_fac['fac_id']) . "' CLASS='plain text' style='float: none; color: #000000;' onMouseOver=\"do_hover(this.id);\" onMouseOut=\"do_plain(this.id);\" onClick = 'do_fac_mail_win(" . intval($row_fac['fac_id']) . ", \"" . e($row_fac['security_email']) . "\");'>Email</SPAN>";
        } else {
        $tomail = "";
        }
    }
...
$theTabs .= '<span id="tab3" class="tab" style="cursor: pointer;" onClick="do_tab(\'tab3\', 3, ' . floatval($row_fac['lat']) . ',' . floatval($row_fac['lng']) . ');">Location</span>';
$theTabs .= '</div>';
...
if(!(isempty(trim($row_fac['access_rules']))))      {$line_ctr++; $tab_1 .= "<TR CLASS='even'><TD CLASS='td_label text' ALIGN='right'>" . get_text("Access rules") . ":&nbsp;</TD><TD CLASS='td_data_wrap text' ALIGN='left'>" . e(str_replace($eols, " ", $row_fac['access_rules'])) . "</TD></TR>";}
if(!(isempty(trim($row_fac['security_reqs']))))      {$line_ctr++; $tab_1 .= "<TR CLASS='odd'><TD CLASS='td_label text' ALIGN='right'>Security reqs:&nbsp;</TD><TD CLASS='td_data_wrap text' ALIGN='left'>" . e(str_replace($eols, " ", $row_fac['security_reqs'])) . "</TD></TR>";}
...
if ($use_twitter) {                            //7/23/15
    $theInformation = e($row_fac['facility_name']) . " at " . $address . ", " . e($description) . ". <small>Latitude: " . floatval($row_fac['lat']) . ", Longitude: " . floatval($row_fac['lng']) . "</small>";
    $tab_1 .=     "<TR style='height: 25px;'>
        <TD style='text-align: center;'>
            <IMG id='twit_" . intval($fac_id) . "' class='plain text' SRC='./buttons/tweetbutton.png' style='float: none; margin: 0px; padding: 0px; vertical-align: middle;'
...
if (my_is_float($row_fac['lat'])) {                                        // position data of any type?
    ...
    $tab_2 .= "<TR><TD class='td_label text' style='font-size: 80%;'>Lat</TD><TD class='td_data text' style='font-size: 80%;'>" . floatval($row_fac['lat']) . "</TD></TR>";
    $tab_2 .= "<TR><TD class='td_label text' style='font-size: 80%;'>Lng</TD><TD class='td_data text' style='font-size: 80%;'>" . floatval($row_fac['lng']) . "</TD></TR>";
