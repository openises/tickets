...
<style type="text/css">
	*, html { margin:0; padding:0 }
	...
	div#app_outer { position: absolute; top: 0px; left: 0%; width: 100%; height: auto; z-index: 99999; color: #000000;}
	...
	div#center_but { z-index: 5; display: inline-block; position: fixed; top: 110px; right: 110px; }
	div#toggle_tracks_but { z-index: 5; position: fixed; top: 160px; right: 110px; }	
	div#toggle_tracks_off_but { z-index: 5; position: fixed; top: 160px; right: 110px; }
	div#day_but { z-index: 5; display: inline-block; position: fixed; top: 210px; right: 110px; }
	div#night_but { z-index: 5; display: inline-block; position: fixed; top: 210px; right: 110px; }		
	div#map_controls { z-index: 5; display: inline-block; position: fixed; top: 100px; right: 90px;}
	div#map_but { z-index: 5; display: inline-block; position: fixed; top: 60px; right: 5px; width: 60px; height: 60px; }			
	div#plus_but { z-index: 5; display: inline-block; position: fixed; top: 200px; right: 20px; }
	div#minus_but { z-index: 5; display: inline-block; position: fixed; top: 250px; right: 20px; }
	...
	.screen { z-index: 5; position: relative; left: 5%; top: 20px; width: 90%; height: 50%; background: transparent;}
	.chat_screen { z-index: 5; position: relative; left: 5%; top: 20px; width: 90%; height: 50%; background: transparent;}	
...
<SCRIPT SRC='../js/leaflet-openweathermap.js' TYPE="text/javascript"></SCRIPT>
...
function set_size() {
	...
	var listHeight = viewportheight * .5;
...
function the_status(status, title) {
	...
	if (confirm("Are you sure you want submit this " + title + " report?")) { 
		do_geolocate(latLng, the_lat, the_lng);
		setTimeout(function() {sub_data(title,form_add,the_lat,the_lng,status); get_conditions(); alert("Conditions Report Submitted"); },2000);
		}
	}
...
function get_geo() {	//	Start up the geo location
	...
	setTimeout(do_loop(), 5000);
	}
	
function do_loop() {
	primary_timer = window.setInterval('primary_repeats()', 30000);	// the shortest timer, get new location data
	...
	secondary_timer = window.setInterval('secondary_repeats()', 60000);	// 30 second timer - chat, messages and markers
...
<body onUnload='stop_loop();'>
	<DIV id='outer' style='height: 100%%; width: 100%;'>
		<DIV id="buttons" style='position: relative; top: 65px; left: 0px; text-align: left; width: 80%; display: block; z-index: 9999;'>
<?php
			if($logged_in == 0) {
?>
...
?>
			...
			<BR /><BR />
			<DIV id='statuses' style='position: relative; left: 5px; text-align: left; width: 100%; display: block;'>
				<TABLE style='width: 100%;'>
<?php
					if($the_responder != 0) {
?>
						<TR style='width: 100%; text-align: left;'>
							<TD class='text text_left' STYLE='width: 20%;'>My Status</TD>
							<TD class='text text_left' STYLE='width: 35%;'><?php print $the_status_sel;?></TD>
						</TR>
<?php
						}
					if(count($c_types > 0)) {
?>
						<TR style='width: 100%;'>
							<TD class='text text_left' STYLE='width: 20%;'>Hazard Reports</TD>
							<TD class='text text_right' STYLE='width: 35%;'>
...
?>
		...
		<DIV id='map_controls' style='display: none; border: 3px outset #707070; background-color: #CECECE; width: 70px; height: 170px; z-index: 99999;'>
...
?>		
		...
		<DIV id='screen2' class='screen' style='display: none;'>
			<DIV id="alert_list" class='lists' style='padding: 10px;'></DIV>	
		...
		<DIV id='screen3' class='screen' style='display: none;'>
			<DIV id="ticket_list" class='lists' style='padding: 10px;'></DIV>
		...
		<DIV id='screen4' class='screen' style='display: none;'>
			<DIV id="message_list" class='lists' style='padding: 10px;'></DIV>	
		...
		<DIV id='screen5' class='chat_screen' style='display: none;'>
