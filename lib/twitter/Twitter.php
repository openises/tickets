...
class Twitter
{
	...
	/**
	 * Sends direct message to Twitter user.
	 * @param  string   message encoded in UTF-8
	 * @return stdClass  see https://dev.twitter.com/rest/reference/post/direct_messages/new
	 * @throws TwitterException
	 */
	public function direct($message, $userid = NULL, $screenname = NULL)
	{
		return $this->request(
			'direct_messages/new',
			'POST',
			array('text' => $message, 'screen_name' => $screenname, 'user_id' => $userid)
		);
	}

	/**
	 * Gets direct message to Twitter user.
	 * @param  string   message encoded in UTF-8
	 * @return stdClass  see https://dev.twitter.com/rest/reference/post/direct_messages/new
	 * @throws TwitterException
	 */
	public function rec_direct($limit = NULL)
	{
		return $this->request(
			'direct_messages',
			'GET',
	 ...
	 * @throws TwitterException
	 */
	public function sent_direct($limit = NULL)
	{
		return $this->request(
	 ...
	 * @throws TwitterException
	 */
	public function load($flags = self::ME, $count = 40, array $data = NULL)
	{
	...
	public static function clickable($status)
	{
		...
		foreach ($all as $pos => $item) {
			$s = iconv_substr($s, 0, $pos, 'UTF-8')
				. '<a target="_blank" href="' . htmlspecialchars($item[0]) . '">' . htmlspecialchars($item[1]) . '</a>'
